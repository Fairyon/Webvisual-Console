...
public class ExecHelper implements Runnable {
	 ...
	 * 
	 * @param testFrame
	 *            MainFrame, that need Infos from the Process
	 * @param p
	 *            Process, that need to be logged
	 * @param n
	 *            true if testFrame.serverStopped Funktion needs to be called
	...
	/**
	 * Must be called after process Termination call
	 * handler.serverStopped(exitValue) if notify is true
	 */
	...
	// Prepend a current Data to the input, and call handler.processNewInput()
	// with it
	private void processNewInput(String input) {
		// Handle process new input..
		handler.processNewInput(
				(!input.equals("") ? new SimpleDateFormat("yyyy/MM/dd HH:mm:ss").format(new Date()) + " " : "")
						+ input);
	}
	 ...
	 *            MainFrame, that need Infos from the Process
	 * @param p
	 *            Process, that need to be handled
	 * @param n
	 *            true if testFrame.serverStopped Funktion needs to be called
	 *            after Process termination
	 */
	 ...
	 *            MainFrame, that need Infos from the Process
	 * @param command
	 *            Command with arguments, that needs to be executed
	 * @param n
	 *            true if testFrame.serverStopped Funktion needs to be called
	 *            after Process termination
	 */
	...
	public void run() {
		...
		if (Thread.currentThread() == processThread) {
			try {
				// This Thread just waits for the process to end and notifies
				// the handler..
